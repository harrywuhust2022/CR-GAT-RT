...
class BaseDataSet(Dataset):
    def __init__(self, root, split, mean, std, base_size=None, augment=True, val=False,
                crop_size=321, scale=True, flip=True, rotate=False, blur=False, noise=False,return_id=False):
        self.root = root
        self.split = split
        self.mean = mean
        self.std = std
    ...
    def _augmentation(self, image, label):
        ...
        if self.noise:
            sigma=5
            gaussian = np.random.normal(0,sigma/(255*0.24) ,image.shape)
    ...
    def __getitem__(self, index):
        ...
        return self.to_tensor(image1), label
