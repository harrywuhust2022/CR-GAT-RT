...
from tools import calculate_correct_map
...
class Trainer(BaseTrainer):
    ...
    def _train_CR(self, epoch):
        ...
        for batch_idx, (data, target) in enumerate(tbar):
            ...
            # PRINT INFO
            tbar.set_description('TRAIN ({}) | Loss: {:.3f} | Acc {:.2f} mIoU {:.2f} | B {:.3f} D {:.3f} |'.format(
                                                epoch, self.total_loss.average, 
                                                pixAcc, mIoU,
                                                loss1, loss2))
    ...
    def _train_fastadt(self, epoch):
        ...
        for batch_idx, (data, target) in enumerate(tbar):
            ...
            def step(x, g,alpha):
                l = len(x.shape) - 1
                g_norm = torch.norm(g.view(g.shape[0], -1),1,dim=1).view(-1, *([1]*l))
            ...
            delta = torch.zeros_like(data).uniform_(-max_perturb, max_perturb).cuda()
            ...
            delta.data =step(delta.data,grad,alpha).renorm(p=1, dim=0, maxnorm=eps) 
    ...
    def _valid_epoch(self, epoch):
        ...
        with torch.no_grad():
            ...
            for batch_idx, (data, target) in enumerate(tbar):
                ...
                # PRINT INFO
                pixAcc, mIoU, _ = self._get_seg_metrics().values()
                tbar.set_description('EVAL ({}) | PixelAcc: {:.2f}, Mean IoU: {:.2f} |'.format( epoch,
